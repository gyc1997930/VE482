...
std::string CopyTableQuery::toString() {
    return "QUERY = Copytable TABLE, "
                   "Table = \""+this->tableName1+"\", "
                   "ToTable = \""+this->tableName2+"\"";
}

QueryResult::Ptr CopyTableQuery::execute() {
    Database &db=Database::getInstance();
    auto &table1=db[this->tableName1];
    try {
        auto table2=table1;
    }catch(const std::exception &e) {
        return std::make_unique<ErrorMsgResult>(qname, e.what());
...
QueryResult::Ptr DropTableQuery::execute() {
    ...
    try {
        ...
        return make_unique<ErrorMsgResult>(qname, string(e.what()));
    }
...
QueryResult::Ptr DumpTableQuery::execute() {
    ...
    if(!ofile.is_open()) {
        return std::make_unique<ErrorMsgResult>(qname, tableName.c_str(), R"(Cannot open File "?".)"_f%this->fileName);
    }
    Database &db=Database::getInstance();
    try {
        ofile<<(db[this->tableName]);
    }catch(const std::exception &e) {
        return std::make_unique<ErrorMsgResult>(qname, tableName.c_str());
    }
    ofile.close();
    return std::make_unique<SuccessMsgResult>(qname, tableName.c_str());
}
...
QueryResult::Ptr PrintTableQuery::execute() {
    ...
    try {
        ...
        cout<<table;
        cout<<"================\n"<<endl;
        return make_unique<SuccessMsgResult>(qname, tableName.c_str());
    }catch(const TableNameNotFound &e) {
        return make_unique<ErrorMsgResult>(qname, this->tableName.c_str(), "No such table."s);
    }
}
